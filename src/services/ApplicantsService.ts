...
export const createApplicantApi = async (
  ...
  try {
    ...
    const { data } = await apiClient.post("/api/applicants", {
      first_name,
      middle_name,
      last_name,
      gender_id,
    ...
    console.error("Error on create applicant:", error.message);
    const errorMessage =
      error.response?.data?.message || "An unexpected error occurred";
...
export async function getApplicantByIdApi(id: string): Promise<{ data: Applicant }> {
  try {
