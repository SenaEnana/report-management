...
export const uploadBranchReportApi = async (
  file: File | undefined,
  transaction_date: string
) => {
  if (!file) throw new Error("No file selected");
  ...
  localStorage.setItem("lastBranchReportData", JSON.stringify(processedData));

  return true;
};

export const updateExchangeRateApi = async (
  CUP: number,
  MC: number,
  VC: number,
  date: string,
