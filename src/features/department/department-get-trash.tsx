...
import { getTrashedDepartmentApi, restoreDepartmentApi } from "@/services/DepartmentService";
...
export default function DepartmentGetTrash() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await getTrashedDepartmentApi(
                pageIndex,
                pageSize,
                searchQuery
            )
            ...
            console.error("Error fetching department trashed resources:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleRestoreDepartment = async (data: any) => {
        try {
            const departmentId = data.id;
            const response = await restoreDepartmentApi(departmentId);
            console.log("Restore response:", response);
            if (response && response.id === departmentId) {
                setData((prevData) => prevData.filter((item) => item.id !== departmentId));
                setTotalItems((prevTotal) => prevTotal - 1);
                toast({
                    title: "Success!",
                    description: `${data.name} department restored!`,
                });
            } else {
                toast({
                    title: "Error",
                    description: "Failed to restore department. Please try again.",
                    variant: "destructive",
                });
            }
        } catch (error: any) {
    ...
    const columnsConfig: {
        ...
        cell?: ({ row }: { row: any }) => JSX.Element;
            ...
            { accessorKey: "name", title: "Department Name" },
    ...
    const mappedData = data.map((department) => {
        const faculty = faculties?.find((f: { id: string }) => f.id === department.faculty_id);
        return {
            ...department,
        ...
        columns: [
            ...
            })),
            {
                ...
                cell: ({ row }: { row: any }) => (
                    <ActionDropdown
                        type="trash"
                        onRestore={() => { handleRestoreDepartment(row.original) }}
                    />
                ),
            },
        ],
