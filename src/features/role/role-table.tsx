...
import { fetchRoleApi, filterRoleApi, getRoleColumnApi, softDeleteRoleApi } from "@/services/RoleService";
...
const columnsConfig = [
    { accessorKey: "name", title: "Role Name" },
...
export default function RoleTable() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetchRoleApi(pageIndex, pageSize, searchQuery);
    ...
    useEffect(() => {
        const fetchColumns = async () => {
            try {
                const columns = await getRoleColumnApi();
    ...
    const handleApplyFilters = async () => {
        try {
            setLoading(true);
            const { filteredData, totalItems } = await filterRoleApi(
                pageSize,
                filters
            );
    ...
    const handleEditRole = (data: any) => {
        try {
            ...
            navigate(`/user/role/view/edit/${data.id}`, { state: data });
        } catch (error) { }
    };

    const handleViewRole = (data: any) => {
        console.log("Navigating with data:", data);
        try {
            navigate(`/user/role/view/detail/${data.id}`, { state: data });
        } catch (error) {
    ...
    const handleDeleteRole = async (role: Role) => {
        try {
            const isSoftDeleted = await softDeleteRoleApi(role.id);
            if (isSoftDeleted) {
                setData((prevData) => prevData.filter((item) => item.id !== role.id));
                setTotalItems((prevTotal) => prevTotal - 1);
                toast({
                    title: "Success!",
                    description: `${role.name} role deleted!`,
                });
            } else {
                toast({
                    title: "Error",
    ...
    const table = useReactTable({
        ...
        columns: [
            ...
            })),
            {
                ...
                cell: ({ row }: { row: any }) => (
                    <ActionDropdown
                        onView={() => { handleViewRole(row.original) }}
                        onEdit={() => {
