...
import { getTrashedUserApi, restoreUserApi } from "@/services/UserService";
...
export default function UserGetTrash() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await getTrashedUserApi(
                pageIndex,
                pageSize,
                searchQuery
            )
    ...
    const handleRestoreUser = async (data: any) => {
        try {
            const userId = data.id;
            const response = await restoreUserApi(userId);
            ...
            if (response && response.id === userId) {
                ...
                toast({
                    title: "Success!",
                    description: `${data.name} user restored!`,
                });
            } else {
                toast({
                    title: "Error",
    ...
    const mappedData = data.map((department) => {
        const gender = genders?.find((f: { id: number }) => f.id === department.gender_id);
        return {
            ...department,
        ...
        columns: [
            ...
            })),
            {
                ...
                cell: ({ row }: { row: any }) => (
                    <ActionDropdown
                        type="trash"
                        onRestore={() => { handleRestoreUser(row.original) }}
                    />
                ),
            },
        ],
