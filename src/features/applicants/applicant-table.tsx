...
import { fetchApplicants, filterApplicantApi, getApplicantColumnApi, softDeleteApplicantApi } from "@/services/ApplicantsService";
...
const columnsConfig: {
  ...
  cell?: ({ row }: { row: any }) => JSX.Element;
    ...
    { accessorKey: "gender_name", title: "Gender" },
  ];
...
export default function ApplicantTable() {
  ...
  useEffect(() => {
    const fetchColumns = async () => {
      try {
        const columns = await getApplicantColumnApi();
  ...
  const handleApplyFilters = async () => {
    try {
      setLoading(true);
      const { filteredData, totalItems } = await filterApplicantApi(
        pageSize,
        filters
      );
  ...
  const handleEditApplicant = (data: any) => {
    try {
      ...
      navigate(`/applicant/view/edit/${data.id}`, { state: data });
    } catch (error) { }
  };
  ...
  const handleViewApplicant = (data: any) => {
    ...
    try {
      navigate(`/applicant/view/detail/${data.id}`, { state: data });
    } catch (error) {
  ...
  const handleDeleteApplicant = async (data: any) => {
    try {
      ...
      const isSoftDeleted = await softDeleteApplicantApi(applicantId);
      if (isSoftDeleted) {
        // setPageIndex(1);
        ...
        toast({
          title: "Success!",
          description: `${data.first_name} applicant deleted!`,
        });
      } else {
        toast({
          title: "Error",
  ...
  const mappedData = data.map((applicant) => {
    const gender = genders?.find((f: { id: number; name: string }) => f.id === applicant.gender_id);
    return {
      ...applicant,
      gender_name: gender ? gender.name : "Unknown gender",
    };
  });
