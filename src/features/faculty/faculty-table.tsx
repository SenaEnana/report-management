...
export default function FacultyTable() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetchFacultiesApi(
        pageIndex,
        pageSize,
        searchQuery
      );
  ...
  useEffect(() => {
    const fetchColumns = async () => {
      try {
        const columns = await getFacultyColumnApi();
  ...
  const handleApplyFilters = async () => {
    try {
      setLoading(true);
      const { filteredData, totalItems } = await filterFacultyApi(
        pageSize,
        filters
      );
  ...
  const handleEditFaculty = (data: any) => {
    try {
      console.log("data", data);
      navigate(`/faculty/view/edit/${data.id}`, { state: data });
    } catch (error) { }
  };

  const handleViewFaculty = (data: any) => {
    console.log("Navigating with data:", data);
    try {
      navigate(`/faculty/view/detail/${data.id}`, { state: data });
    } catch (error) {
      console.error("Error navigating to detail view:", error);
    }
  };

  const handleDeleteFaculty = async (data: any) => {
    try {
      console.log("data", data.id);
      const facultyId = data.id;
      const isSoftDeleted = await softDeleteFacultyApi(facultyId);
      if (isSoftDeleted) {
        // setPageIndex(1);
        ...
        toast({
          title: "Success!",
          description: `${data.name} faculty deleted!`,
        });
      } else {
        toast({
          title: "Error",
  ...
  const table = useReactTable({
    ...
    columns: [
      ...
      })),
      {
        ...
        cell: ({ row }: { row: any }) => (
          <ActionDropdown
            onView={() => { handleViewFaculty(row.original); }}
            onEdit={() => {
              handleEditFaculty(row.original);
            }}
            onDelete={() => {
              handleDeleteFaculty(row.original);
            }}
          />
        ),
      },
